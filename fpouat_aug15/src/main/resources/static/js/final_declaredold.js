...
function raiseqry(data) {
	...
	var resObj = $.ajax({
		...
		success: function(developerDatas) {
			...
			window.location.href = "import_query?id=" + $(data).attr('data-cin');
		},
		fail: function(rs, e) {
...
function updassover() {
	$('.updateAss').attr("disabled", false);//changed
	$('.updatecancel').attr("disabled", true);
	//$('.updateAss').removeClass('blink1');
	$('.updateAss').addClass('blink1');//changed
}
...
function dispothrs(e) {
	...
	$(".blueClass").show();
	$(".yellowClass").hide();
	/*$("#customDropDown1").attr("disabled", true);
	$("#customDropDown1").attr("disabled", true);
	$("#notn1").attr("disabled", true);
	$("#slno1").attr("disabled", true);
	$("#dutyAmount1").attr("disabled", true);
	$("#dutyAmountFg1").attr("disabled", true);
	$("#addrow1").attr("disabled", true);*/
	
	$("#customDropDown1").attr("disabled", false);//changed
	$("#customDropDown1").attr("disabled", false);//changed
	$("#notn1").attr("disabled", false);//changed
	$("#slno1").attr("disabled",false);//changed
	$("#dutyAmount1").attr("disabled", false);//changed
	$("#dutyAmountFg1").attr("disabled", false);//changed
	$("#addrow1").attr("disabled", false);//changed
	...
	$.ajax({
		...
		success: function(data) {
			...
			if (null == data1) {
				...
				//	------------------------------------------ // changed for next page button start here --------------------------------
			...
			$('.updateAss').attr("disabled", false); //changed from true to false
			$('.updateAss').addClass('blink1');//changed from removeclass to addclass
			$('#changeButton').attr("disabled", false);
			//$('#nextPageButton').attr("disabled", false);
			$('#1').attr("disabled", false);//changed from true to false
			$('#getOtherFourDigi').attr("disabled", false);//changed from true to false
			$('#getOtherEightDigi').attr("disabled", false);//changed from true to false
			$('#getOtherDigit').attr("disabled", false);//changed from true to false
			$('#addrow1').attr("disabled", false);//changed from true to false
			$('#addrow7').attr("disabled", true);
			$('#bothAssQry').attr("disabled", true);
			$("#qryShHd").hide();
		},
...
function exmorder(cinNo) {
	window.location.href = "imp_order?id=" + cinNo;
}
	...
	success: function(fpoItemDetOthDutyList) {
		...
		if (null != fpoItemDetOthDutyList && fpoItemDetOthDutyList != "") {
				...
				if (null != item.duty_CD && item.duty_CD != "" && !(item.duty_CD.toString() == 'Select')) {
					...
					$.ajax({
						...
						success: function(fpoItemDetOthDutys) {
							...
							$("#customDropDown" + selCount + " option:contains(" + fpoItemDetOthDutys['cdDesc'] + ")").attr('selected', 'selected');
							$("#customDropDown" + selCount).attr("disabled", false); // changed from true to false
							selCount = selCount + 1;
						},
						fail: function(rs, e) {
							console.log(rs, responseText);
			...
			//		$("#slno" + custCount + " option:contains(" + item.duty_SLNO + ")").attr('selected', 'selected');
					...
					if (!(parseFloat($('#acfirst').val()) == 1 && $('#role').val() == 'PAC')) {
						$("#notn" + custCount).attr("disabled", false);// changed from true to false
						$("#slno" + custCount).attr("disabled", false);// changed from true to false
						$("#1").attr("disabled", true);// changed from true to false
						$("#dutyAmount" + custCount).attr("disabled", false);// changed from true to false
						$("#dutyAmountFg" + custCount).attr("disabled", false);// changed from true to false
						...
						$('.updateAss').attr("disabled", false);// changed from true to false
						$('.updateAss').addClass('blink1');// changed from true to false
						$('#changeButton').attr("disabled",true);// changed from false to true
