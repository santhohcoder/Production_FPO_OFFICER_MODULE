...
function entrssoid(val3) {
	...
	if (val3.value != "") {
		$.ajax({
			url: 'getassignedroles?ssoid=' + val3.value,
			success: function(success) {
               	$('.mailclass').html('');
				if (success != "") {
					$('#selctroles').empty();
					...
					/*	if (value == 'PAO') {
						...
						$.ajax({
							...
							success: function(success) {
								...
								$('.mailclass').append('<li><span style="color: #000000;">Mail Class :</span>' + '<span>&nbsp;&nbsp;-&nbsp;&nbsp;</span>' + " " + '<span style="color: #00000">' + assrole + '</span>' + '<span style="color: blue">' + " " + "-" + " " + joincls + '</span></li>');
							}
						});
					});
					$('.rolesassigned').html('<span style="color: #000000;">Roles Assigned to the user</span>' + '<span>&nbsp;&nbsp;-&nbsp;&nbsp;+ assrole + </span>' + " " + html);
             ...
             $('.mailclass1').html('');
				if (data != "") {
					...
					$.each(data, function(index, value) {
						...
						$.ajax({
							url: 'getmailClas?mailcls=' + value + '&ssoid=' + toid.value,
							success: function(success) {
								var mailcls = success[0].split(',');
								var joincls = "";
								var joinclsArr=[];
								...
								mailclsjson[assothrole]=joinclsArr;
								$('.mailclass1').append('<li><span style="color: #000000;">Mail Class given for</span>' + '<span>&nbsp;&nbsp;-&nbsp;&nbsp;</span>' + " " + '<span style="color: #00000">' + assothrole + '</span>' + '<span style="color: blue">' + " " + "-" + " " + joincls + '</span></li>');
							}
						});
					});
...
function back() {
	...
	var sd = pathNameurl.split("/") 
	var pathName = sd[1];
	if(pathName == "reallocation" ) {
