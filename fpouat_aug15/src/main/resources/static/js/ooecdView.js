...
$("#stcdtable").dataTable({
		...
		buttons: [{extend: 'csvHtml5', text: '<i>EXCEL</i>',
					...
					filename:'OOE Codes Directory',
				  //filename:'Electronic Advance Data (EAD)-SET ASIDE LIST',
					message:'OOE Codes Directory @ ' + sitecode + '\n\r' + datetime,},
				  {extend: 'pdf',
				    text:'<i>PDF</i>',
				    pageSize: 'A4',
                    className: ' btn btn-success mb-3 ',
					title: 'FPO IMPORT APPLICATION - ICES'+
							'\n\r'+'OOE Codes Directory @ ' + sitecode,
					filename:'OOE Codes Directory',
				...
				/*	exportOptions: {
					...
					doc.content[1].table.widths = ['8%','23%','23%','23%','23%'];
		...
		"initComplete": function( settings ) {
	     $("#stcdtable_filter").append('<button type="button" class="btn btn-info" id="clear6" '+
			'style="margin-left: 15px;margin-bottom: 5px;">Clear</button>');
          $("#stcdtable_filter input[type='search']").attr('id', 'searchstcdtable');
		$('#clear6').click(function() {	
		document.getElementById("searchstcdtable").value=""
		var table = $('#stcdtable').DataTable();
		table.search('').draw();
	   })
    	}
