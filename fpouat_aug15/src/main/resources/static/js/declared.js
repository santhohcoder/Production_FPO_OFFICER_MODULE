...
function eadassstop() {
	...
	window.location.href = "ead_submit?id=" + $("#inputPassword").val() + "&cat=ASSSTOP";
}
...
function chkcommsamp() {
	if ($('#natcd').val() == 'Commercial sample')
		$("#commsampPopup").modal('show');
	else
		aclWarningMsg();
}
...
$("#receiptchange").click(function() {
	...
	if ($('#othersDoc').val() != '' && checkbox7.checked) { 			// changed (added) for otherdoc 
		$("#othersModal").modal('show');
	}
});


function validateOthersDoc() {
	var docName = document.getElementById("othersDoc").value;
	if (docName == "") {
...
const inputElement = document.getElementById('othersDoc');


const storedValue = localStorage.getItem('docName');
...
inputElement.addEventListener('input', (event) => {
	...
	localStorage.setItem('docName', inputValue);
});
...
$("#itemdesctab").keyup(function() {
	/* if (($(this).val().length > 5) && ($(this).val() != null) && ($(this).val().trim().length > 5)) 
		$("#receiptchange").attr("disabled", false);
	else
		$("#receiptchange").attr("disabled", true);*/

});

function valueChanged() {
	/*if($('.nextcheck').is(":checked"))   
		$("#remarkchange").attr("disabled", false);
	else
		$("#remarkchange").attr("disabled", true);*/
}

$("#generalRemarks").keyup(function() {
	/* if (($(this).val().length > 5) && ($(this).val() != null) && ($(this).val().trim().length > 5))
		 $("#queryraisebtn").attr("disabled", false);
	 else
		 $("#queryraisebtn").attr("disabled", true);*/
...
function raiseqry(data) {
		var reg =/<(.|\n)*?>/g; 
	 if (reg.test($('#generalRemarks').val()) == true || reg.test($('#query1').val()) == true) {
	     swal('OOPS!', 'Special character Not allowed!!', 'error');
		return false
	...
	if ($('#role').val() == 'PAC') {
		...
		if (processTypeFromList == "for Query Approval") {
	...
	var resObj = $.ajax({
		// url: 'raiseqry?que='+"E",
		url: 'raiseqry?que=' + "E" + '&others=' + docName,
		data: oMyForm,
		processData: false,
		contentType: false,
		type: "post",
...
$("#query" + $('#itemNoChange').val()).keyup(function() {
...
$(document).ready(function() {
	...
	console.log($('#arrFpo').val());
	
	if($('#arrFpo').val() == 'Y'){
		$("#arrFpoModal").modal('show');
	} else {
	if (!$("#backInput").val()) {
		$("#myModal").modal('show');
	 }
	...
	$("#manualduty").show();
	$("#manualdutyO").hide();
	if ($('#role').val() == 'PAC' && ($('#updassdt').val() == null || $('#updassdt').val() == "")) {
		ass_hide();
		$('.updateAss').attr("disabled", true);
	}
	else if (parseFloat($('#acfirst').val()) == 1 && $('#role').val() == 'PAC') {
	...
	/*if (($("#generalRemarks").val().length > 5) && ($("#generalRemarks").val() != null) && ($("#generalRemarks").val().trim().length > 5))
		$("#queryraisebtn").attr("disabled", false);
	else
		$("#queryraisebtn").attr("disabled", true);*/
	...
	if ($('#checkqry').val() != '' && $('#checkqry').val() != undefined && $('#checkqry').val() != null) {
		const myArr = $('#checkqry').val().split(";");

		myArr.forEach(element => $('#checkbox' + element).prop('checked', true));
	}

	if ($("#itemDutyFgChange1").val() != null && $("#itemDutyFgChange1").val() != '') {
...
function calTotDuty(i) {
	...
	var totalDutyCal = 0;
	var totalDutyCalFg = 0;
	var totalDutyFg = 0;
	var prvallitemduty = $('#allItemDuty').val() == '' ? 0 : $('#allItemDuty').val();
	var prvallitemdutyFg = $('#allItemDutyFg').val() == '' ? 0 : $('#allItemDutyFg').val();
	var prvDutyDiff = $('#allItemDuty').val() == '' ? 0 : $('#allItemDuty').val();
	var prvDutyDiffFg = $('#allItemDutyFg').val() == '' ? 0 : $('#allItemDutyFg').val();
	var prvtotitemduty = $('#totalOthersDuty').val() == '' ? 0 : $('#totalOthersDuty').val();
	var prvtotitemdutyfg = $('#totalOthersDutyFg').val() == '' ? 0 : $('#totalOthersDutyFg').val();

	if (i == 1) {
		var prvtotitemduty = $('#itemDutyChange1').val();
		var prvtotitemdutyfg = $('#itemDutyFgChange1').val();
	}
	//if (i==2)
	// {
	...
	$("#tbOtrDt").find("tr").each(function() {
		...
		if (null != $(this).find("td").eq(6).find("input[name*='DUTY_FG']").val() && "" != $(this).find("td").eq(6).find("input[name*='DUTY_FG']").val()) {
			calTotDutyFgArray.push(parseFloat($(this).find("td").eq(6).find("input[name*='DUTY_FG']").val()));
			totalDutyCalFg = totalDutyCalFg + parseFloat($(this).find("td").eq(6).find("input[name*='DUTY_FG']").val());
		}
	});
	...
	$("#enterOthDuty").find("tr").each(function() {
		...
		if (null != $(this).find("td").eq(6).find("input[name*='DUTY_FG']").val() && "" != $(this).find("td").eq(6).find("input[name*='DUTY_FG']").val()) {
			...
			totalDutyCalFg = totalDutyCalFg + parseFloat($(this).find("td").eq(6).find("input[name*='DUTY_AMOUNT']").val());
		}
	});
	...
	if (num > 1) {
		...
		var totalItemDutyFg = $('#totalOthersDutyFg').val();
		var basicTotalItemDuty = $('#itemDutyChange1').val();
		var basicTotalItemDutyFg = $('#itemDutyFgChange1').val();
		var totalDiff = (parseFloat(totalDutyDiff) - parseFloat(basicTotalItemDuty)).toFixed(2);
		var totalDiffFg = (parseFloat(totalDutyDiffFg) - parseFloat(basicTotalItemDutyFg)).toFixed(2);
		$('#allItemDuty').val((parseFloat(prvallitemduty) + parseFloat(totalItemDuty) - parseFloat(prvtotitemduty)).toFixed(2));
		$('#allItemDutyFg').val((parseFloat(prvallitemdutyFg) + parseFloat(totalItemDutyFg) - parseFloat(prvtotitemdutyFg)).toFixed(2));
	} else {
		$('#allItemDuty').val($('#totalOthersDuty').val());
		$('#allItemDutyFg').val($('#totalOthersDutyFg').val());
	}
	//}
	setFlagReq('O');
	calTotPayable();
...
function changeDetails(value, opt) {
	if (opt == 1) {
		alert("AMENDMENT IS NOT ALLOWED AT Electronic Advance Data (EAD) STAGE.");
...
function updateAmendment(id) {
	...
	$.ajax({
		...
		success: function(developerDatas) {
			...
			$("#nextItemPegination2").text(developerData['item_NO']);
			...
			// $("#query" + $('#itemNoChange').val()).val(developerData['query']);
			...
			$("#itemSwNoNtChange1").append($('<option>', {
				text: developerData['sw_NOTN']
			}));
			$("#itemSwNoNtChange1 option:contains(" + developerData['sw_NOTN'] + ")").attr('selected', 'selected');
						...
						if (null != item.duty_CD && item.duty_CD != "" && !(fpoItemDetOthDutyList[stRow].duty_CD.toString() == 'Select')) {
							...
							$.ajax({
								...
								success: function(fpoItemDetOthDutys) {
									...
									$("#customDropDown" + selCount).attr("disabled", false); // changed for reupdate aaf
									selCount = selCount + 1;
								},
								fail: function(rs, e) {
									console.log(rs, responseText);
							...
							$("#notn" + custCount).attr("disabled", false); // changed for reupdate aaf

							$('#slno' + custCount).append($('<option>', {
								text: item.duty_SLNO
							}));
							$("#slno" + custCount + " option:contains(" + item.duty_SLNO + ")").attr('selected', 'selected');
							$("#slno" + custCount).attr("disabled", false); // changed for reupdate aaf
							...
							$("#1").attr("disabled", false); // changed for reupdate aaf
							$("#dutyAmount" + custCount).attr("disabled", false); // changed for reupdate aaf
							$("#dutyAmountFg" + custCount).attr("disabled", false); // changed for reupdate aaf

							//   $('#bothAssQry').attr("disabled", true);
							$('#addrow1').attr("disabled", false); // changed for reupdate aaf
							//    $('.updateAss').attr("disabled", true);
							$('#changeButton').attr("disabled", false);
							...
							$("#enterDropDown" + (enterCount + 1)).attr("disabled", false); // changed for reupdate aaf
							$("#enterRateDuty" + (enterCount + 1)).attr("disabled", true);
							$("#enterNotn" + (enterCount + 1)).attr("disabled", false); // changed for reupdate aaf
							$("#enterSlno" + (enterCount + 1)).attr("disabled", false); // changed for reupdate aaf
							$("#enterEffRate" + (enterCount + 1)).attr("disabled", true);
							$("#enterDutyAmount" + (enterCount + 1)).attr("disabled", false); // changed for reupdate aaf
							$("#dutyFgAmt" + (enterCount + 1)).attr("disabled", false); // changed for reupdate aaf
...
$.ajax({
	...
	success: function(datas) {
		...
		$.each(datas, function(i, item) {
			$("#itemPagination" + item + "").text("" + (i + 1) + "/" + $("#noOfItems").text());
		});
	},
	fail: function(rs, e) {
		console.log(rs, responseText);
...
function dispothrs(e) {
	...
	$(".blueClass").show();
	$(".yellowClass").hide();
	$("#customDropDown1").attr("disabled", false); // changed for reupdate aaf
	$("#customDropDown1").attr("disabled", false); // changed for reupdate aaf
	$("#notn1").attr("disabled", false); // changed for reupdate aaf
	$("#slno1").attr("disabled", false); // changed for reupdate aaf
	$("#dutyAmount1").attr("disabled", false); // changed for reupdate aaf
	$("#dutyAmountFg1").attr("disabled", false); // changed for reupdate aaf
	$("#addrow1").attr("disabled", false); // changed for reupdate aaf
...
$("#assinfobtn").click(function() {
	$('#assinfomodal').modal('hide');
	var opt = 0;
	if (!($('#itemBcdNsNoChange1').val() == null) && !($('#itemBcdNsNoChange1').val() == 'Select SNO'))
		opt = 1;
	if (!($('#itemIgstNsNoChange1').val() == null) && !($('#itemIgstNsNoChange1').val() == 'Select SNO'))
	...
	if ((opt == 1) || (opt == 2)) {
		...
		updateAssessment($('#updateqryid').val(), $('#updateqryval').val());
...
function updateOthersData() {
	...
	$("#tbOtrDt").find("tr").each(function() {
		...
		if (null != fpoItemDetOthDuty['duty_CD'] && fpoItemDetOthDuty['duty_CD'] != "" && !(fpoItemDetOthDuty['duty_CD'].toString().startsWith('Select Duty'))) {
			...
			fpoItemDetOthDuty['ass_DT'] = new Date();
			fpoItemDetOthDuty['query'] = $("#query" + $('#itemNoChange').val()).val();
			fpoItemDetOthDuty['totAllAmount'] = $('#totalOthersDuty').val();
			fpoItemDetOthDuty['allItemAmount'] = $('#allItemDuty').val();
			fpoOthersList.push(fpoItemDetOthDuty);
		}
	...
	$.ajax({
		...
		success: function(data) {
			...
			$("#tbOtrDt").find("tr").each(function() {
				$(this).find("td").eq(0).find("select[name*='DUTY']").attr("disabled", false); // changed for reupdate aaf
				$(this).find("td").eq(1).find("input[name*='RATE_OF_DUTY']").attr("disabled", false); // changed for reupdate aaf
				$(this).find("td").eq(2).find("select[name*='NOTN']").attr("disabled", false); // changed for reupdate aaf
				$(this).find("td").eq(3).find("select[name*='SL.NO']").attr("disabled", false); // changed for reupdate aaf
				$(this).find("td").eq(4).find("input[name*='EFF.RATE']").attr("disabled", false); // changed for reupdate aaf
				$(this).find("td").eq(5).find("input[name*='DUTY_AMOUNT']").attr("disabled", false); // changed for reupdate aaf
				$(this).find("td").eq(6).find("input[name*='DUTY_FG']").attr("disabled", false); // changed for reupdate aaf
				$(this).find("td").eq(7).find("input[name*='DELETE']").attr("disabled", false);// changed for reupdate aaf
			});
			...
			//  $('.updateAss').attr("disabled", true);
			$('#changeButton').attr("disabled", false); // changed for reupdate aaf
			//$('#nextPageButton').attr("disabled", false);
			$('#1').attr("disabled", false); // changed for reupdate aaf
			$('#getOtherFourDigi').attr("disabled", false); // changed for reupdate aaf
			$('#getOtherEightDigi').attr("disabled", false);// changed for reupdate aaf
	...
	$.ajax({
		...
		success: function(developerDatas) {
			...
			if (developerDatas['sw_NOTN'] != null && developerDatas['sw_NSNO'] != null) {
				$("#itemSwNoNtChange1 option:contains(" + developerDatas['sw_NOTN'] + ")").attr('selected', 'selected');
				$("#itemSwNsNoChange1").append($('<option>', {
			...
			$("#query" + $('#itemNoChange').val()).text(developerData['query']);
					...
					if (null != developerDatas.fpoItemDetOthDuty[stRow].duty_CD && developerDatas.fpoItemDetOthDuty[stRow].duty_CD != "" && !(developerDatas.fpoItemDetOthDuty[stRow].duty_CD.toString() == 'Select')) {
						...
						$.ajax({
							...
							success: function(datas) {
								...
								$("#customDropDown" + (dropCount + 1) + "").attr("disabled", false); // changed for reupdate aaf
								$("#rateDuty" + (dropCount + 1) + "").val(developerDatas.fpoItemDetOthDuty[dropCount].duty_RTA);
								$("#rateDuty" + (dropCount + 1) + "").attr("disabled", false); // changed for reupdate aaf
								$("#effRate" + (dropCount + 1) + "").val(developerDatas.fpoItemDetOthDuty[dropCount].rate);
								$("#effRate" + (dropCount + 1) + "").attr("disabled", false); // changed for reupdate aaf
								$("#dutyAmount" + (dropCount + 1) + "").val(developerDatas.fpoItemDetOthDuty[dropCount].duty_AMT);
								$("#dutyAmount" + (dropCount + 1) + "").attr("disabled", false); // changed for reupdate aaf
								$("#dutyAmountFg" + (dropCount + 1) + "").val(developerDatas.fpoItemDetOthDuty[dropCount].duty_FG);
								$("#dutyAmountFg" + (dropCount + 1) + "").attr("disabled", false); // changed for reupdate aaf
								...
								$("#notn" + (dropCount + 1) + "").attr("disabled", false); // changed for reupdate aaf
								...
								$("#slno" + (dropCount + 1) + "").attr("disabled", false); // changed for reupdate aaf
								...
								if (null != developerDatas && null != developerDatas['ass_DT'] && developerDatas.fpoItemDetOthDuty.length != 0 && new Date(developerDatas['ass_DT']) < new Date(developerDatas.fpoItemDetOthDuty[0].ass_DT)) {
									$("#1 option:contains(" + "Other than 9804" + ")").prop("selected", false); // changed for reupdate aaf
									$('#1').attr("disabled", false); // changed for reupdate aaf
									$(".blueClass").show();
									$(".yellowClass").hide();
								} else {
...
function presubmitquery() {
	/* if ($("#query" + $('#itemNoChange').val()).val() == "") {
		 $("#bothAssQry").attr("disabled", true);
		 $("#bothAssQry").prop('checked', false);
 
	 }*/
	if (null != $("#query" + $('#itemNoChange').val()).val() && !$("#query" + $('#itemNoChange').val()).val() == "") {
		//   $("#bothAssQry").attr("disabled", false);
		var developerData = {};
		developerData['cinNo'] = $('#inputPassword').val();
		developerData['itemId'] = $('#itemId').val();
		...
		var resObj = $.ajax({
			...
			success: function(developerDatas) {
				...
				if (null == developerDatas['ass_DT'] && null == developerDatas['amend_DT']) {
					...
					// $("#query" + $('#itemNoChange').val()).attr("disabled", false);
					//$("#queryraisebtns").attr("disabled", false);
					//$('.updateAss').attr("disabled", false);
					//    $('.changeAss').attr("disabled", true);
				}
				if ($("#allItemDuty").val() > $('#dutylim').val()) {
...
/*if (null != $("#query" + $('#itemNoChange').val()).text() && $('#bothAssQry').val() == 'N') {
	$('#calcShHd').hide();
	$("#query" + $('#itemNoChange').val()).attr("disabled", true);
				//$("#queryraisebtns").attr("disabled", true);
	$(".updateAss").attr("disabled", true);
		...
		if (null != fpoItemDetOthDutyList && fpoItemDetOthDutyList != "") {
			...
			$.each(fpoItemDetOthDutyList, function(i, item) {
				if (null != item.duty_CD && item.duty_CD != "" && !(item.duty_CD.toString() == 'Select')) {
					...
					if (!(parseFloat($('#acfirst').val()) == 1 && $('#role').val() == 'prw')) {
						$("#notn" + custCount).attr("disabled", false); // changed for reupdate aaf
						$("#slno" + custCount).attr("disabled", false); // changed for reupdate aaf
						$("#1").attr("disabled", false); // changed for reupdate aaf
						$("#dutyAmount" + custCount).attr("disabled", false); // changed for reupdate aaf
...
$(document).ready(function() {
	//	$("#firstBtn").attr("disabled", false);//testing
	//	$("#firstBtn").hide(); 


	if (($('#role').val() == 'PAO' && $('#acloffid').val() != '') && ($("#qryNo").val()) > 0) {
		$("#firstBtn").show();
	...
	if ($('#role').val() == 'PAC') {
		if (parseInt($("#qryNo").val()) > 0)
			$("#queryraisebtns").text('Approve / Modify Query')
		else
			$("#queryraisebtns").remove();
		if ((parseInt($("#qryNo").val()) > 0) && ($('#updassdt').val() == null || $('#updassdt').val() == ""))
			...
			$('#aclpopupmsg').html("AO has done the primary assessment / raised query." + '<BR>' + "AC may alter AO's assesment / Query if required and complete the process.");
		else if ((parseInt($("#qryNo").val()) == 0) && ($('#updassdt').val() != null || $('#updassdt').val() != ""))
			$('#aclpopupmsg').html("AO has done the primary assessment." + '<BR>' + "AC may alter AO's assesment if required and complete the process.");

		var processTypeFromList = localStorage.getItem("processType");

		if (processTypeFromList == "for Query Approval") {
			var toBlink = $('#queryraisebtns');
