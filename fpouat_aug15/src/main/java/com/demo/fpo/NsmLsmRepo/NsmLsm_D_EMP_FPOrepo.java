...
public interface NsmLsm_D_EMP_FPOrepo extends JpaRepository<D_EMP_FPO, Long> {
	
	@Query(nativeQuery = true, value = "select a.user_id,a.cus_site,a.role_name,b.EMP_NAME from ops$dir.d_emp_roles a, ops$dir.d_emp_fpo b where a.user_id=b.user_id and a.user_id=:id and passwd = :pwd and end_dt is null")
	List<Collection> getempName(@Param("id") String id,@Param("pwd") String pwd);
	...
	@Query(nativeQuery = true, value = "select 'images/'||lower(ROLE)||'_icon.JPG',a.ROLE,disp_rolenm,decode(b.user_id,:offid,1,0),a.OLDROLE from ops$dir.fpo_roles a left join ops$dir.d_emp_roles b on (a.role=b.role_name and b.cus_site=:cusite) where disp_group = 1 and role<>'PNA' order by dispord")
...
//Nsm without designation
	...
	@Query(nativeQuery = true, value = " SELECT t.DISP_ROLENM,t.ROLE,t.designation FROM OPS$DIR.fpo_roles t where site_access= :siteaces and disp_group = 3 ORDER  BY dispord ")
	...
	@Query(nativeQuery = true, value = "SELECT t.DISP_ROLENM,t.ROLE,t.designation FROM OPS$DIR.fpo_roles t\r\n"
			+ "                 where site_access <> 'INNSA5' and disp_group = 1 \r\n"
			+ "         ORDER  BY dispord")
